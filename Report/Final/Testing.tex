...
\label{sec:uart_communications_testing}
	...
	\begin{figure}[tb]
		\begin{center}
			\includegraphics[width=\figwidth]{testbenches/uart_wave}
		\end{center}
...
\label{sec:software_based_gdp_speed_testing}
	The Gaussian Distance calculations were implemented in a C program in order to compare its performance with that of the FPGA.  It was found that the processor is capable of calculating a single senone score in 10µs on average -- about 100 times slower than the FPGA.  However, this version used double floating point precision, and thus is far more accurate than the FPGA.  Reducing the accuracy and using fixed-point values would definitely increase its speed.  % TODO!! Do this? It wouldn't be hard...
